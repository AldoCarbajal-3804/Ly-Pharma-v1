...
function getEmployeeName(sale) {
    if (typeof sale.empleado === "object" && sale.empleado) {
        return `${sale.empleado.nombres || ""} ${sale.empleado.apellidos || ""}`.trim() || `#${sale.empleado.id || sale.id_empleado || ""}`
    }
    return sale.nombre_empleado || sale.vendedor || (sale.id_empleado ? `#${sale.id_empleado}` : "-")
}

export const AdminSalesTable = ({ sales, onView, onDelete }) => {
            ...
            <table className="w-full text-sm">
                <thead>
                    <tr className="border-b border-gray-100">
                        <th className="text-left py-3 px-2 text-xs font-bold text-gray-400 uppercase tracking-wider">Empleado</th>
                    ...
                    {sales.length === 0 ? (
                        ...
                        <tr key={sale.id_venta ?? sale.id ?? i} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                            <td className="py-3 px-2">
                                <p className="font-medium text-gray-800">{getEmployeeName(sale)}</p>
                            </td>
                            <td className="py-3 px-2">
